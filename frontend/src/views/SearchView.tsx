...
function parseSmartQuery(raw: string, genres: Genre[]) {
  ...
  const genre = genres
    .filter(g => g.genre)
    .sort((a, b) => String(b.genre).length - String(a.genre).length)
    .find(g => text.includes(String(g.genre).toLowerCase()))
  if (genre) { out.genre = String(genre.id); text = text.replace(String(genre.genre).toLowerCase(), ' ') }
...
export function SearchView({ onFilmClick, onBack }: Props) {
  ...
  const buildParams = useCallback((page: number) => {
    const smart = parseSmartQuery(state.query, state.genres)
    const params = new URLSearchParams()
    params.set('page', String(page))
    const keyword = smart.keyword || state.query
    if (keyword) params.set('keyword', keyword)
    const genre = state.filters.genre || smart.genre
    const type = state.filters.type || smart.type
    const yearFrom = state.filters.yearFrom || smart.yearFrom
    const yearTo = state.filters.yearTo || smart.yearTo
    ...
    return params
  }, [state.query, state.filters, state.genres])
  ...
  }, [buildParams, state.query, dispatch])
  ...
  useEffect(() => {
    ...
    totalRef.current = 0
    fetchPage(1, false)
  }, [state.query, state.filters, dispatch]) // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <div className="grid">
        {isLoading
          ...
          : state.items.length === 0 && !state.loading
            ? <div className="state-box"><strong>Ничего не найдено</strong>Попробуйте изменить запрос или фильтры</div>
